<?php
	...
	$section = $_GET['sec'];
	if($_GET['from']=='google'){								//  들어올때 iTunes id인지 판단하는 분기를 만들거나 해야할듯.
		if($id){
		...
		if($id){
			$it_result = mysqli_query($connect,"SELECT rank, day From $table WHERE id_it_".$section."='$id' order by day");
			$ps_query = mysqli_query($connect, "SELECT ps_id From app_info WHERE it_id='$id'");
		}
		$ps_fetch = mysqli_fetch_row($ps_query);
		$ps_id = $ps_fetch[0];
		...
		if($ps_id == ''){
			...
			$ps_result = mysqli_query($connect, "SELECT rank, day From $table WHERE id_ps_".$section."='$ps_id' order by day");
			while ($ps_row = mysqli_fetch_array($ps_result)){
				$ps_data[] = $ps_row['rank'];
			}
		}

		while ($row = mysqli_fetch_array($it_result)) {
		   $it_data[] = $row['rank'];
		   $day[] = '"'.$row['day'].'"';
		}
	}
